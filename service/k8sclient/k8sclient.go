...
var (
	// ErrPXCClusterNotReady The PXC cluster is not in ready state.
	ErrPXCClusterNotReady = errors.New("PXC cluster is not ready")
...
func (c *K8sClient) UpdatePXCCluster(ctx context.Context, params *PXCParams) error {
	...
	if cluster.Status.PXC.Status != pxc.AppStateReady {
		return errors.Wrapf(ErrPXCClusterNotReady, "state is %v", cluster.Status.Status) //nolint:wrapcheck
	}
...
func (c *K8sClient) GetPXCClusterCredentials(ctx context.Context, name string) (*PXCCredentials, error) {
	...
	if cluster.Status == nil || cluster.Status.Status != pxc.AppStateReady {
		return nil, errors.Wrap(ErrPXCClusterNotReady,
